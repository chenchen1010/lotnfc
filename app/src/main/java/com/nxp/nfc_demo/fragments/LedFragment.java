...
public class LedFragment extends Fragment implements OnClickListener {
	...
	static ImageView nxp_pressed_buttons;
	
	static private CheckBox lcd_check;
	static private CheckBox temp_check;
	...
	private void initVariables(View layout) {
		nxp_pressed_buttons = (ImageView) layout.findViewById(R.id.nxp_pressed_buttons);

		lcd_check = (CheckBox) layout.findViewById(R.id.LCD_checkbox);
		temp_check = (CheckBox) layout.findViewById(R.id.Temp_Sensor_checkbox);
		Scroll_check = (CheckBox) layout.findViewById(R.id.Ndef_Scroll_checkbox);
		
		// Scroll is only available when LCD is enabled
		lcd_check.setOnCheckedChangeListener(new OnCheckedChangeListener() {
