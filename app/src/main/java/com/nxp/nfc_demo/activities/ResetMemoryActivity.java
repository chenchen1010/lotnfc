...
public class ResetMemoryActivity extends Activity {
	...
	public void setDataRate(int bytes, long time) {
		if(bytes > 0) {
			...
			((LinearLayout) findViewById(R.id.layoutResetMemoryStatistics)).setVisibility(View.VISIBLE);
			dataRate_callback.setText(readTimeMessage);
		} else {
			// Make the board input layout visible
			((LinearLayout) findViewById(R.id.layoutResetMemoryStatistics)).setVisibility(View.GONE);
