...
public class FlashMemoryActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		dataRate_callback = (TextView) findViewById(R.id.flashfwdata_datarateCallback);
        ...
        if (requestCode == MainActivity.AUTH_REQUEST  && resultCode == RESULT_OK) {
            if(demo != null) {
	            if (demo.isReady()) {
	            	...
	            	try {
	        			if(isAppFW == true) {
	        				bytes_to_flash = readFileAssets(indexFW);
	        			} else {
	        				String path = ((TextView) findViewById(R.id.file_path)).getText().toString();
	        				bytes_to_flash = readFileMemory(path);
	        			}
	        		} catch (IOException e) {
	        			e.printStackTrace();
	        			
	        			// Set bytes_to_flash to null so that the task is not started
	        			bytes_to_flash = null;
	        		}

	        		if(bytes_to_flash == null || bytes_to_flash.length == 0) {
	...
	private class flashTask extends AsyncTask<Intent, Integer, Boolean> {
		...
		protected void onPostExecute(Boolean success) {
			...
			flashCompleted(success, bytes_to_flash.length, timeToFlashFirmware);
		...
		protected Boolean doInBackground(Intent... nfc_intent) {
			...
			boolean success = demo.Flash(bytes_to_flash);
